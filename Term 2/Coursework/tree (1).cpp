...
TreeNode* find_node(tree_t e, TreeNode* t){
        if (t == NULL) {
            //the subtree val doesnt exist/node doesnt exist therefore  Edgenode*next is  null
            return NULL;
        }
        if (t->val == e) {
            //if there already is a subtree return that subtree so that we can chain subtrees using edgenodes proprly
            return t;
        }
        else {
            EdgeNode* it = t->subtree_l; //same implementation as the print func using it to go through all subtrees
            TreeNode* result; //tree node used to recursively check all subtrees
            while (it != NULL) {
                //recusively check ,requirments already written above
                result = find_node(e, it->subtree);
                if (result != NULL) {
                    // We found the val/node we are looking for in one of the subtrees return immedelty so we can build on it/ conenct edgenodes to it by returning it
                    return result;
                }
                //loop through all it values ( simialar to the print function which must be done in while loop)
                it = it->next;
            }
            // The node was not found in any of the subtrees so edgenode *next msut point to null, lopped throigh all it and nothings been returned therefore we msut return null
            return NULL;
        }
...
void add_tree_node(tree_t s, tree_t d, TreeNode* t){
        ...
        t=find_node(s,t);//tree_root pointer will store its address in the tree_root if it already exits. If the subtree s isnt found in the tree, returns null(as done in func), then the function has pointer next = NULL as we want or just stores the tree pointer.
        ...
        t->subtree_l= cons_edge_node(subtree, t->subtree_l);//we construct an edge node where the edgenode*subtree pointer points to the treenode val we are passign through(the new one we jsut made) as the subtree param and the edgenode pointer *next points to the current edgenode (making a chian of edgenodes) or it points to NULL if there isn't already the value/vegtable in the tree.
